/**
 * Found Items listing page with search and filter.
...
function FoundItems() {
  ...
  async function loadItems() {
    try {
      const data = await itemsApi.getAll({ type: 'found' });
      setItems(data);
    } catch (err) {
  ...
  return (
    <div>
      ...
      {filtered.length === 0 ? (
        <div className="card">No found items reported.</div>
      ) : (
      ...
      {deleteId && (
        <ConfirmDialog
          title="Delete Item"
          message="Are you sure you want to delete this found item?"
